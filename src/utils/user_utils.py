...
# ============================================================
# ОСНОВНЫЕ ФУНКЦИИ ПОЛЬЗОВАТЕЛЕЙ (ТЕКСТОВЫЙ ФОРМАТ)
# ============================================================
...
def load_users():
    ...
    try:
        ...
        logger.error(f"❌ Ошибка загрузки users.txt: {e}")
    
    return users


def save_users(users):
    """Сохраняет список пользователей в текстовый файл."""
    try:
        os.makedirs(os.path.dirname(USERS_FILE), exist_ok=True)
        with open(USERS_FILE, 'w', encoding='utf-8') as f:
            for u in users:
                username = u.get('username', '') or ''
                full_name = u.get('full_name', 'Неизвестный')
                role = u.get('role', '0')
                extra = u.get('extra', '')
                f.write(f"{u['id']}|{username}|{full_name}|{role}|{extra}\n")
        return True
    except Exception as e:
        logger.error(f"❌ Ошибка сохранения users.txt: {e}")
        return False
