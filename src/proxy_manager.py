...
class ProxyManager:
    ...
    async def ping_proxy_once(self, proxy: str, timeout: int = None) -> Optional[float]:
    ...
    async def ping_proxy_triple(self, proxy: str, timeout: int = None) -> Optional[float]:
        """
        Пингует прокси PING_COUNT раз. Возвращает среднее.
        Если PING_COUNT == 1 — просто возвращает пинг.
        """
        if timeout is None:
            timeout = self.PING_TIMEOUT
